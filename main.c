...
#pragma pack(pop) // Restaura o alinhamento padrão do sistema
...
// Converte string para maiúsculas (compatibilidade C puro)
...
// Formata nome FAT (8.3) para exibição convencional
void format_name(char *dest, const char *src_name, const char *src_ext) {
    int i, idx = 0;

    for (i = 0; i < 8; i++) {
        if (src_name[i] != ' ' && src_name[i] != '\0') dest[idx++] = src_name[i];
    }

    if (src_ext[0] != ' ' && src_ext[0] != '\0') {
        dest[idx++] = '.';
        for (i = 0; i < 3; i++) {
            if (src_ext[i] != ' ' && src_ext[i] != '\0') dest[idx++] = src_ext[i];
        }
    }
    dest[idx] = '\0';
}

// Converte nome convencional (ex: TESTE.TXT) para o padrão estrutural da FAT (8+3 bytes)
void parse_to_fat_name(const char *input, char *fat_name, char *fat_ext) {
    memset(fat_name, ' ', 8);
    memset(fat_ext, ' ', 3);
    
    char *dot, *ext, temp[16];
    int i;

    strncpy(temp, input, 15);
    temp[15] = '\0';
    to_uppercase(temp);
    dot = strchr(temp, '.');
...
// Calcula o offset em bytes de um cluster na área de dados
uint32_t get_cluster_offset(const FAT16_Context *ctx, uint16_t cluster) {
    return ctx->data_area_offset + ((cluster - 2) * ctx->boot_record.sectors_per_cluster * ctx->boot_record.bytes_per_sector);
}

// Lê o valor de uma entrada na tabela FAT
uint16_t read_fat_entry(FAT16_Context *ctx, uint16_t cluster) {
    uint16_t next_cluster;
    fseek(ctx->disk_file, ctx->fat_offset + (cluster * 2), SEEK_SET);
    fread(&next_cluster, sizeof(uint16_t), 1, ctx->disk_file);
    return next_cluster;
}

// Escreve um valor na tabela FAT
void write_fat_entry(FAT16_Context *ctx, uint16_t cluster, uint16_t value) {
    fseek(ctx->disk_file, ctx->fat_offset + (cluster * 2), SEEK_SET);
    fwrite(&value, sizeof(uint16_t), 1, ctx->disk_file);
    
    // Atualiza a FAT2 se ela existir para manter a integridade
    if (ctx->boot_record.fat_count > 1) {
        uint32_t fat2_offset = ctx->fat_offset + (ctx->boot_record.sectors_per_fat * ctx->boot_record.bytes_per_sector);
        fseek(ctx->disk_file, fat2_offset + (cluster * 2), SEEK_SET);
        fwrite(&value, sizeof(uint16_t), 1, ctx->disk_file);
    }
}

int init_filesystem(FAT16_Context *ctx, const char *image_path) {
    ctx->disk_file = fopen(image_path, "rb+");

    if (!ctx->disk_file) {
        printf("Erro ao abrir a imagem de disco: %s\n", image_path);
        return 0;
    }

    fread(&(ctx->boot_record), sizeof(FAT16_BootRecord), 1, ctx->disk_file);

    if (ctx->boot_record.boot_sector_signature != 0xAA55) {
        printf("Assinatura de boot inválida!\n");
        fclose(ctx->disk_file);
        ctx->disk_file = NULL;
        return 0;
    }

    ctx->fat_offset = ctx->boot_record.reserved_sectors * ctx->boot_record.bytes_per_sector;
    ctx->root_dir_offset = ctx->fat_offset + (ctx->boot_record.fat_count * ctx->boot_record.sectors_per_fat * ctx->boot_record.bytes_per_sector);
    ctx->data_area_offset = ctx->root_dir_offset + (ctx->boot_record.root_entry_count * sizeof(FAT16_DirEntry));
